...
**FastAPI + Streamlit + LangGraph/LangChain + LangMem + ChromaDB + Groq + SQLite**.
...
| [7](#phase-7--copilot-orchestration-the-brain) | Memory + RAG + tools + Groq → draft | 90 min |
...
- [ ] *(Optional but recommended)* A **Google AI Studio API key** for Gemini embeddings
- [ ] Docker Desktop (needed only from Phase 11)
- [ ] A GitHub account (needed only from Phase 12)

> **Note on the optional Google key:** without it, the system still works. RAG falls back to Chroma's built-in embedding function and memory falls back to "list recent memories" instead of semantic search. Build the fallback path deliberately — it is one of the strongest things you can talk about in an interview.
...
uv add langchain langchain-core langgraph langchain-groq
uv add langmem
uv add chromadb langchain-text-splitters
uv add langchain-google-genai google-genai
...
# Optional — enables Gemini embeddings for RAG + semantic memory
GOOGLE_API_KEY=
...
class Settings(BaseSettings):
    ...
    def semantic_enabled(self) -> bool:
        """True when a Google key is present, so we can embed text."""
...
```bash
python -c "from customer_support_agent.core.settings import get_settings; s=get_settings(); print(s.app_name, s.db_path, s.semantic_enabled)"
```

Expected: the app name, an absolute `data/app.db` path, and `True`/`False`. The `data/` folder now exists.
...
def _get_embedder():
    """Return a LangChain embeddings object, or None to use Chroma's default."""
    global _embedder
    s = get_settings()
...
def get_store() -> InMemoryStore:
    """Singleton store. Vector-indexed when a Google key is configured."""
    global _store
...
| Condition | Behaviour |
|---|---|
| Google key set, memories exist, semantic hit | Ranked semantic results |
| Google key set, no semantic match | Falls back to recent-memory listing |
| No Google key | Plain store, recent-memory listing only |
...
Expected: at least one result. With semantic search on you get a real score; without it you get `0.0` scores from the recent-listing fallback. **Both are a pass.**
...
**`customer_support_agent/integrations/llm/groq_client.py`**
...
from customer_support_agent.core.settings import get_settings
from customer_support_agent.integrations.llm.groq_client import get_llm
from customer_support_agent.integrations.memory import langmem_store as mem
from customer_support_agent.integrations.rag import chroma_kb
...
def generate_recommendation(ticket: dict, customer: dict) -> tuple[str, dict[str, Any]]:
    ...
    context["model"] = settings.groq_model
...
cp .env.example .env
nano .env                # paste real GROQ_API_KEY / GOOGLE_API_KEY
...
| Symptom | Cause | Fix |
|---|---|---|
| `GROQ_API_KEY is not set` | `.env` not loaded | Run commands from the project root; check `env_file=".env"` in settings |
...
| Memory search always returns score `0.0` | No `GOOGLE_API_KEY` | Expected fallback behaviour, not a bug |
...
| Rate limit / 429 from Groq | Free-tier limits | Lower `rag_top_k`, shorten prompts, or add backoff |
